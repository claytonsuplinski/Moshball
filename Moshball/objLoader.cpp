...
bool ObjLoader::Initialize(const char * objName, const char * jpgName, char* v, char* f)
{
	if (this->GLReturnedError("ObjLoader::Initialize - on entry"))
		return false;
...
if (!this->PostGLInitialize(&this->vertex_array_handle, &this->vertex_coordinate_handle, this->vertices.size() * sizeof(VertexAttributes), &this->vertices[0]))
	...
	if (this->GLReturnedError("ObjLoader::Initialize - on exit"))
		return false;
...
//Delete the object
void ObjLoader::TakeDown()
{
	super::TakeDown();
}

//Draws the object with respect to the lighting
void ObjLoader::Draw(const mat4 & projection, mat4 modelview, const ivec2 & size, const float time)
{
	if (this->GLReturnedError("ObjLoader::Draw - on entry"))
		return;
	...
	if (this->GLReturnedError("ObjLoader::Draw - on exit"))
		return;
}